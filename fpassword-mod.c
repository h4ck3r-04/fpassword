...
char pair[260];
char *FPASSWORD_EXIT = "\x00\xff\x00\xff\x00";
char *FPASSWORD_EMPTY = "\x00\x00\x00\x00";
              ...
              else
              {
                ...
                if (buf[1] != 0)
                {
                  ...
                  if (buf[1] == 5)
                    ...
                    fpassword_report(stderr, "[ERROR] SOCKS error %d\n", buf[1]);
                  err = 1;
                }
              }
            }
...
char *fpassword_get_next_pair()
{
  if (pair[0] == 0)
  {
    ...
    if (__fck >= sizeof(FPASSWORD_EXIT) && memcmp(&FPASSWORD_EXIT, &pair, sizeof(FPASSWORD_EXIT)) == 0)
      return FPASSWORD_EXIT;
  }
  return pair;
}
...
void fpassword_report_found(int32_t port, char *svc, FILE *fp)
{
  /*
    ...
    port, svc, fpassword_get_next_login()); else fprintf(fp, "[%d][%s] login: %s\n",
    port, svc, fpassword_get_next_login()); else if (colored_output) fprintf(fp,
    "[\e[31m%d\e[0m][\e[31m%s\e[0m] login: \e[32m%s\e[0m   password:
    \e[32m%s\e[0m\n", port, svc, fpassword_get_next_login(),
    fpassword_get_next_password()); else fprintf(fp, "[%d][%s] login: %s   password:
    %s\n", port, svc, fpassword_get_next_login(), fpassword_get_next_password());
    ...
    fpassword_get_next_login(), fpassword_get_next_password());
    }
...
void fpassword_report_pass_found(int32_t port, char *ip, char *svc, FILE *fp)
{
  /*
    ...
    \e[32m%s\e[0m\n", port, svc, ipaddr_str, fpassword_get_next_password()); else
      fprintf(fp, "[%d][%s] host: %s   password: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_password()); if (stdout != fp) printf("[%d][%s] host: %s
    password: %s\n", port, svc, ipaddr_str, fpassword_get_next_password());
    fflush(fp);
  */
}
...
void fpassword_report_found_host(int32_t port, char *ip, char *svc, FILE *fp)
{
  /*  char *keyw = "password";
    ...
    \e[32m%s\e[0m\n", port, svc, ipaddr_str, fpassword_get_next_login()); else
        fprintf(fp, "[%d][%s] host: %s   login: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_login()); else if (!strcmp(svc, "snmp3")) if (colored_output)
        fprintf(fp, "[\e[31m%d\e[0m][\e[31m%s\e[0m] host: \e[32m%s\e[0m   login:
    \e[32m%s\e[0m\n", port, svc, ipaddr_str, fpassword_get_next_password()); else
        fprintf(fp, "[%d][%s] host: %s   login: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_password()); else if (!strcmp(svc, "cisco-enable") ||
    !strcmp(svc, "cisco")) if (colored_output) fprintf(fp,
    "[\e[31m%d\e[0m][\e[31m%s\e[0m] host: \e[32m%s\e[0m   password:
    \e[32m%s\e[0m\n", port, svc, ipaddr_str, fpassword_get_next_password()); else
        fprintf(fp, "[%d][%s] host: %s   password: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_password()); else if (colored_output) fprintf(fp,
    "[\e[31m%d\e[0m][\e[31m%s\e[0m] host: \e[32m%s\e[0m   login: \e[32m%s\e[0m
    %s: \e[32m%s\e[0m\n", port, svc, ipaddr_str, fpassword_get_next_login(), keyw,
              fpassword_get_next_password());
    else
      fprintf(fp, "[%d][%s] host: %s   login: %s   %s: %s\n", port, svc,
    ipaddr_str, fpassword_get_next_login(), keyw, fpassword_get_next_password()); if
    (stdout != fp) { if (!strcmp(svc, "rsh") || !strcmp(svc, "oracle-sid"))
        printf("[%d][%s] host: %s   login: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_login()); else if (!strcmp(svc, "snmp3")) printf("[%d][%s]
    host: %s   login: %s\n", port, svc, ipaddr_str, fpassword_get_next_password());
      else if (!strcmp(svc, "cisco-enable") || !strcmp(svc, "cisco"))
        printf("[%d][%s] host: %s   password: %s\n", port, svc, ipaddr_str,
    fpassword_get_next_password()); else printf("[%d][%s] host: %s   login: %s   %s:
    %s\n", port, svc, ipaddr_str, fpassword_get_next_login(), keyw,
    fpassword_get_next_password());
    }
    fflush(fp);
...
void fpassword_report_found_host_msg(int32_t port, char *ip, char *svc, FILE *fp, char *msg)
{
  /*
    ...
    fpassword_get_next_login(), fpassword_get_next_password(), msg); else fprintf(fp,
    "[%d][%s] host: %s   login: %s   password: %s  [%s]\n", port, svc,
    ipaddr_str, fpassword_get_next_login(), fpassword_get_next_password(), msg); if
    (stdout != fp) printf("[%d][%s] host: %s   login: %s   password: %s\n",
    port, svc, ipaddr_str, fpassword_get_next_login(), fpassword_get_next_password());
    fflush(fp);
  */
}
...
int32_t fpassword_connect_ssl(char *host, int32_t port, char *hostname)
{
  if (__first_connect != 0)
    ...
    sleep(conwait);
#ifdef LIBOPENSSL
  return (internal__fpassword_connect_ssl(host, port, SOCK_STREAM, 6, hostname));
#else
  fprintf(stderr, "Error: not compiled with SSL\n");
  return -1;
#endif
        ...
        if (debugbuf != NULL) {
          for (k = 0; k < size; k++)
            if (buf[k] == 0)
              ...
              debugbuf[k] = buf[k];
          debugbuf[size] = 0;
          fpassword_report_debug(stderr, "DEBUG_SEND_BEGIN|%s|END [pid:%d]",
       debugbuf, getpid()); free(debugbuf);
        }*/
  }

  /*    if (fpassword_data_ready_writing(socket)) < 1) return -1; XXX maybe needed
   * in the future */
