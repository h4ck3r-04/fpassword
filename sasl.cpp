...
char *sasl_plain(char *result, char *login, char *pass) {
  ...
  if (2 * strlen(preplogin) + 3 + strlen(preppasswd) < 180) {
    ...
    fpassword_tobase64((unsigned char *)result, strlen(preplogin) * 2 + strlen(preppasswd) + 2, 250);
  }
  free(preplogin);
  free(preppasswd);
  return result;
...
char *sasl_digest_md5(char *result, char *login, char *pass, char *buffer, char *miscptr, char *type, char *webtarget, int32_t webport, char *header) {
      ...
      if (strstr(array[i], "\"") != NULL) {
        ...
        if ((nonce_string_len > 0) && (nonce_string_len <= sizeof(nonce) - 1)) {
          ...
          for (j = 0; j < ind; j++)
            if (array[j] != NULL)
              free(array[j]);
          fpassword_report(stderr, "Error: DIGEST-MD5 nonce from server could not be extracted\n");
          result = NULL;
          return result;
        }
      } else {
      ...
      if (strstr(array[i], "\"") != NULL) {
        ...
        if ((realm_string_len > 0) && (realm_string_len <= sizeof(realm) - 1)) {
          ...
          for (i = 0; i < ind; i++)
            if (array[i] != NULL)
              free(array[i]);
          fpassword_report(stderr, "Error: DIGEST-MD5 realm from server could not be extracted\n");
          result = NULL;
          return result;
        }
      } else {
    ...
    if (strstr(array[i], "algorithm=") != NULL) {
      ...
      if ((strstr(algo, "MD5") == NULL) && (strstr(algo, "md5") == NULL)) {
        ...
        for (j = 0; j < ind; j++)
          if (array[j] != NULL)
            free(array[j]);
        fpassword_report(stderr, "Error: DIGEST-MD5 algorithm not based on md5, based on %s\n", algo);
        result = NULL;
        return result;
      }
    }
  ...
  if (!auth_find)
    ...
    snprintf(buffer, 500, "%s:%s:%s:%s:%s", nonce, "00000001", "fpassword", "auth", buffer2);
  MD5_Init(&md5c);
        ...
        if (strstr(type, "rtsp") != NULL) {
          ...
          if (use_proxy == 1 && proxy_authentication[selected_proxy] != NULL)
            snprintf(result, 500,
                     ...
                     "nonce=\"%s\", cnonce=\"fpassword\", nc=00000001, algorithm=%s, "
