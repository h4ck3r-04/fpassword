...
#define EXIT_PROTOCOL_ERROR fpassword_child_exit(2)
...
bool smb2_run_test(creds_t *cr, const char *server, uint16_t port) {
  ...
  */
  ...
  case ENOENT:
    ...
    fpassword_report(stderr, "[WARNING] %s might accept any credential\n", server);
  case EINVAL: // 22
  ...
  case ECONNREFUSED:
    ...
    fpassword_report(stderr, "[ERROR] Error %s (%d) while connecting to %s\n", strerror(errno), errno, server);
    smbc_free_context(ctx, 1);
    EXIT_CONNECTION_ERROR;
    break;
  default:
    // unexpected error
    fpassword_report(stderr, "[ERROR] %s (%d)\n", strerror(errno), errno);
    smbc_free_context(ctx, 1);
    EXIT_PROTOCOL_ERROR;
  }
...
int32_t service_smb2_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  while (*miscptr) {
    ...
    fpassword_report(stderr, "[ERROR] unable to parse: %s\n", miscptr);
    return -1;
  }
...
void usage_smb2(const char *service) {
  puts("Module is a thin wrapper over the Samba client library (libsmbclient).\n"
       ...
       "Fpassword module), the accuracy is reduced. That is, this module works by\n"
       ...
       "\n"
       "Examples: \n"
       "  fpassword smb2://abc.com -l admin -p xxx -m workgroup:{OFFICE}\n"
       "  fpassword smb2://1.2.3.4 -l admin -p F54F3A1D3C38140684FF4DAD029F25B5 -m "
       "'workgroup:{OFFICE} nthash:true'\n"
       "  fpassword -l admin -p F54F3A1D3C38140684FF4DAD029F25B5 "
