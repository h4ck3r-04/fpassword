...
int32_t start_http(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp, char *type, ptr_header_node ptr_head)
{
  ...
  case AUTH_DIGESTMD5:
  {
    ...
    pbuffer = fpassword_strcasestr(http_buf, "WWW-Authenticate: Digest ");
  ...
  case AUTH_NTLM:
  {
    ...
    pos = fpassword_strcasestr(http_buf, "WWW-Authenticate: NTLM ");
    if (pos != NULL)
    {
      char *str;
    ...
    {
      fpassword_report(stderr, "[ERROR] It is not NTLM authentication type\n");
      return 3;
    }
  ...
  {
#ifdef HAVE_PCRE
    if (end_condition_type >= 0 && fpassword_string_match(http_buf, end_condition) != end_condition_type)
    {
#else
...
#endif
      if (debug)
        fpassword_report(stderr, "End condition not match continue.\n");
      fpassword_completed_pair();
    }
    else
    {
      if (debug)
        fpassword_report(stderr, "END condition %s match.\n", end_condition);
      fpassword_report_found_host(port, ip, "www", fp);
  ...
  else
  {
    ...
    if ((fpassword_strcasestr(http_buf, "WWW-Authenticate: Basic") == NULL) && (http_auth_mechanism == AUTH_BASIC))
    {
      // seems the auth supported is not Basic scheme so testing further
      int32_t find_auth = 0;

      if (fpassword_strcasestr(http_buf, "WWW-Authenticate: NTLM") != NULL)
      {
        http_auth_mechanism = AUTH_NTLM;
        find_auth = 1;
      }
#ifdef LIBOPENSSL
      if (fpassword_strcasestr(http_buf, "WWW-Authenticate: Digest") != NULL)
      {
...
void service_http(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname, char *type)
{
  ...
  if (!parse_options(optional1,
                     &ptr_head)) // this function is in fpassword-http-form.c !!
    run = 4;
...
int32_t service_http_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname)
{
  ...
  if (start != NULL)
  {
    ...
    {
      fpassword_report(stderr, "Condition string cannot be bigger than %u.", END_CONDITION_MAX_LEN);
      return -1;
    }
    // copy condition witout starting string (F= or S=  2char)
    strncpy(end_condition, start + 2, condition_len - 2);
    if (debug)
      fpassword_report(stderr, "End condition is %s, mod is %d\n", end_condition, end_condition_type);

    if (*(start - 1) == ' ')
      start--;
    memset(start, '\0', condition_len);
    if (debug)
      fpassword_report(stderr, "Modified options:%s\n", miscptr);
  }
  else
  {
    if (debug)
      fpassword_report(stderr, "Condition not found\n");
  }
