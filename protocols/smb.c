...
http://technet.microsoft.com/en-us/library/cc960646.aspx
   ...
   This code allows Fpassword to directly test NTLM hashes against
...
int32_t NBSSessionRequest(int32_t s)
{
  ...
  memcpy(nb_local, "EIFJEEFCEBCACACACACACACACACACACA", 32); /* FPASSWORD */
  ...
  fpassword_send(s, buf, 72, 0);
  free(buf);

  memset(rbuf, 0, 400);
  k = fpassword_recv(s, (char *)rbuf, sizeof(rbuf));
...
int32_t SMBNegProt(int32_t s)
{
  ...
  fpassword_send(s, (char *)buf, iLength, 0);
  k = fpassword_recv(s, (char *)rbuf, sizeof(rbuf));
  if (k == 0)
    return 3;
...
unsigned long SMBSessionSetup(int32_t s, char *szLogin, char *szPassword, char *miscptr)
{
  ...
  if (security_mode == ENCRYPTED)
  {
    ...
    else if (smb_auth_mechanism == AUTH_NTLMv2)
    {
      ...
      if (verbose)
        fpassword_report(stderr, "[VERBOSE] HashNTLMv2 response length: %d\n", iByteCount);
  ...
  if (verbose)
    fpassword_report(stderr, "[VERBOSE] Set NBSS header length: %2.2X\n", buf[3]);

  /* Set data byte count */
  buf[iOffset - 2] = iByteCount;
  if (verbose)
    fpassword_report(stderr, "[VERBOSE] Set byte count: %2.2X\n", buf[57]);

  fpassword_send(s, (char *)buf, iOffset + iByteCount, 0);

  nReceiveBufferSize = fpassword_recv(s, bufReceive, sizeof(bufReceive));
...
int32_t start_smb(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp)
{
  ...
  strncpy(ipaddr_str, fpassword_address2string(ip), sizeof(ipaddr_str) - 1);
  ...
  if (verbose)
    fpassword_report(stderr, "[VERBOSE] SMBSessionRet: %8.8X SMBerr: %4.4X SMBaction: %2.2X\n", (uint32_t)SMBSessionRet, SMBerr, SMBaction);
...
int32_t service_smb_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname)
{
  ...
  time_t ctime;
  int ready = 0, sock = fpassword_connect_tcp(ip, port);
...
void usage_smb(const char *service)
{
  printf("Module smb default value is set to test both local and domain account, "
         ...
         "Example: \n"
         "      fpassword smb://microsoft.com  -l admin -p tooeasy -m \"local lmv2\"\n"
         "      fpassword smb://microsoft.com  -l admin -p "
         "D5731CFC6C2A069C21FD0D49CAEBC9EA:2126EE7712D37E265FD63F2C84D2B13D::: -m "
         "\"local hash\"\n"
         "      fpassword smb://microsoft.com  -l admin -p tooeasy -m "
