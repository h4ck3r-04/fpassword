...
int32_t start_pop3(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp) {
  ...
  case AUTH_CRAMSHA256: {
    ...
    if (buf[0] != '+') {
      ...
      case AUTH_CRAMMD5:
        ...
        break;
      case AUTH_CRAMSHA1:
        fpassword_report(stderr, "[ERROR] POP3 CRAM-SHA1 AUTH : %s\n", buf);
        break;
      case AUTH_CRAMSHA256:
        fpassword_report(stderr, "[ERROR] POP3 CRAM-SHA256 AUTH : %s\n", buf);
        break;
      }
      free(buf);
      return 3;
  ...
  case AUTH_NTLM: {
    ...
    if (fpassword_send(s, buffer, strlen(buffer), 0) < 0)
      return 1;
    if ((buf = fpassword_receive_line(s)) == NULL || strlen(buf) < 6)
      return 4;
...
void service_pop3(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  if (list_remove(p) != 0)
    fpassword_report(stderr, "[ERROR] Could not find ip %s in pool to free memory\n", fpassword_address2string(ip));
...
int32_t service_pop3_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  if (verbose) {
    ...
    case AUTH_CLEAR:
      ...
      break;
    case AUTH_LOGIN:
      fpassword_report(stderr, "[VERBOSE] using POP3 LOGIN AUTH mechanism\n");
      break;
    case AUTH_PLAIN:
      fpassword_report(stderr, "[VERBOSE] using POP3 PLAIN AUTH mechanism\n");
      break;
    case AUTH_APOP:
#ifdef LIBOPENSSL
      if (strlen(apop_challenge) == 0) {
...
#ifdef LIBOPENSSL
    case AUTH_CRAMMD5:
      fpassword_report(stderr, "[VERBOSE] using POP3 CRAM-MD5 AUTH mechanism\n");
      break;
    case AUTH_CRAMSHA1:
      fpassword_report(stderr, "[VERBOSE] using POP3 CRAM-SHA1 AUTH mechanism\n");
      break;
    case AUTH_CRAMSHA256:
      fpassword_report(stderr, "[VERBOSE] using POP3 CRAM-SHA256 AUTH mechanism\n");
      break;
    case AUTH_DIGESTMD5:
      fpassword_report(stderr, "[VERBOSE] using POP3 DIGEST-MD5 AUTH mechanism\n");
      break;
#endif
    case AUTH_NTLM:
      fpassword_report(stderr, "[VERBOSE] using POP3 NTLM AUTH mechanism\n");
      break;
    }
  }
