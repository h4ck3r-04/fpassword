...
Fpassword Form Module
-----------------

The fpassword form can be used to carry out a brute-force attack on simple
...
./fpassword -l "<userID>" -P pass.txt 10.221.64.12 http-post-form
"/irmlab2/testsso-auth.do:ID=^USER^&Password=^PASS^:Invalid Password"

./fpassword -S -s 443 -l "<username>" -P pass.txt 10.221.64.2 https-get-form
...
#include "include/fpassword-http.h"
...
void hdrrep(ptr_header_node *ptr_head, char *oldvalue, char *newvalue)
{
  ...
  for (cur_ptr = *ptr_head; cur_ptr; cur_ptr = cur_ptr->next)
  {
    if ((cur_ptr->type == HEADER_TYPE_USERHEADER || cur_ptr->type == HEADER_TYPE_USERHEADER_REPL) && strstr(cur_ptr->value, oldvalue))
    {
      ...
      {
        fpassword_report(stderr, "[ERROR] Out of memory (hddrep).\n");
...
void hdrrepv(ptr_header_node *ptr_head, char *hdrname, char *new_value)
{
  ...
  for (cur_ptr = *ptr_head; cur_ptr; cur_ptr = cur_ptr->next)
  {
    if ((cur_ptr->type == HEADER_TYPE_DEFAULT) && strcmp(cur_ptr->header, hdrname) == 0)
    {
      ...
      {
        fpassword_report(stderr, "[ERROR] Out of memory (hdrrepv %lu)\n", strlen(new_value) + 1);
        fpassword_child_exit(0);
      }
    }
  }
...
int32_t parse_options(char *miscptr, ptr_header_node *ptr_head)
{
  ...
  while (*miscptr != 0)
  {
    ...
    {
      fpassword_report(stderr, "[ERROR] optional parameters must have the format X=value: %s\n", miscptr);
      return 0;
    }
    switch (miscptr[0])
    {
    ...
    case 'C':
      ...
      if (*ptr != 0)
        *ptr++ = 0;
      sprintf(cookieurl, "%.1000s", fpassword_strrep(miscptr + 2, "\\:", ":"));
      miscptr = ptr;
      break;
    case '1':
    ...
    case 'h':
       ...
       */
      if (add_header(ptr_head, miscptr + 2, fpassword_strrep(ptr, "\\:", ":"), HEADER_TYPE_USERHEADER))
      {
      ...
      fpassword_report(stderr, "[ERROR] Out of memory for HTTP headers (h).\n");
      return 0;
    case 'H':
       ...
       *  - ptr contains the header's value
       */
      if (add_header(ptr_head, miscptr + 2, fpassword_strrep(ptr, "\\:", ":"), HEADER_TYPE_USERHEADER_REPL))
      {
        // Success: break the switch and go ahead
        miscptr = ptr2;
        break;
      ...
      return 0;
    default:
      fpassword_report(stderr, "[ERROR] no valid optional parameter type given: %c\n", miscptr[0]);
      return 0;
    }
  }
  return 1;
...
char *html_encode(char *string)
{
  ...
  if (strchr(ret, '%') != NULL)
    ret = fpassword_strrep(ret, "%", "%25");
  if (strchr(ret, ' ') != NULL)
    ret = fpassword_strrep(ret, " ", "%20");
  if (strchr(ret, '&') != NULL)
    ret = fpassword_strrep(ret, "&", "%26");
  if (strchr(ret, '#') != NULL)
    ret = fpassword_strrep(ret, "#", "%23");
  if (strchr(ret, '=') != NULL)
    ret = fpassword_strrep(ret, "=", "%3D");
  if (strchr(ret, '+') != NULL)
    ret = fpassword_strrep(ret, "+", "%2B");
...
int32_t analyze_server_response(int32_t s)
{
  ...
  redirected_flag = 0;
  auth_flag = 0;
  while ((buf = fpassword_receive_line(s)) != NULL)
  {
    runs++;
    ...
    if (fpassword_strcasestr(buf, "Location: ") != NULL)
    {
      char *startloc, *endloc;
      char str[2048];

      startloc = fpassword_strcasestr(buf, "Location: ") + strlen("Location: ");
    ...
    if (fpassword_strcasestr(buf, "Set-Cookie: ") != NULL)
    {
      char *cookiebuf = buf;
      ...
      do
      {
        ...
        startcookie = fpassword_strcasestr(cookiebuf, "Set-Cookie: ") + strlen("Set-Cookie: ");
        ...
        if (strchr(startcookie, '=') != NULL && (ptr = strchr(startcookie, '=')) - startcookie + 1 <= sizeof(tmpname))
        {
          ...
          *(++ptr) = 0;
          ...
          // replaced?)
          if ((ptr = fpassword_strcasestr(cookie, tmpname)) != NULL)
          {
            // yes it is.
      ...
      } while (fpassword_strcasestr(cookiebuf, "Set-Cookie: ") != NULL);
    }
#ifdef HAVE_PCRE
    if (fpassword_string_match(buf, cond) == 1)
    {
#else
  ...
  {
    if (debug)
      fpassword_report(stderr, "DEBUG: no response from server\n");
    return -1;
  }
  return 0;
}

void fpassword_reconnect(int32_t s, char *ip, int32_t port, unsigned char options, char *hostname)
{
  if (s >= 0)
...
int32_t start_http_form(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp, char *hostname, char *type, ptr_header_node ptr_head, ptr_cookie_node ptr_cookie)
{
  ...
  fpassword_tobase64((unsigned char *)b64login, strlen(b64login), sizeof(b64login));
  strcpy(b64pass, pass);
  fpassword_tobase64((unsigned char *)b64pass, strlen(b64pass), sizeof(b64pass));
  ...
  upd3variables = fpassword_strrep(variables, "^USER^", clogin);
  upd3variables = fpassword_strrep(upd3variables, "^PASS^", cpass);
  upd3variables = fpassword_strrep(upd3variables, "^USER64^", b64login);
  upd3variables = fpassword_strrep(upd3variables, "^PASS64^", b64pass);
  ...
  if (use_proxy == 1 && proxy_authentication[selected_proxy] != NULL)
  {
    if (getcookie)
    {
      ...
      if (fpassword_send(s, http_request, strlen(http_request), 0) < 0)
        return 1;
      i = analyze_server_response(s); // ignore result
      if (strlen(cookie) > 0)
        process_cookies(&ptr_cookie, cookie);
      fpassword_reconnect(s, ip, port, options, hostname);
    }
  ...
  if (debug)
    fpassword_report_debug(stdout, "HTTP request sent:\n%s\n", http_request);
  ...
  if (auth_flag)
  { // we received a 401 error - user may be using wrong module
    if (code_401_is_failure)
    { // apparently they don't think so  -- treat 401 as failure
  ...
  while (found == 0 && redirected_flag && !code_302_is_success && (redirected_url_buff[0] != 0) && (redirected_cpt > 0))
  {
...
#ifdef HAVE_PCRE
    if (fpassword_string_match(redirected_url_buff, cond) == 1)
    {
#else
    ...
    else
    {
      ...
      else
      {
        ...
        if (debug)
          fpassword_report(stderr, "[DEBUG] host=%s redirect=%s origin=%s\n", str2, str3, url);
      }
      if (str3[0] != '/')
      {
        j = strlen(str3);
      ...
      if (verbose)
        fpassword_report(stderr, "[VERBOSE] Page redirected to http[s]://%s%s\n", str2, str3);
  ...
  {
    fpassword_report_found_host(port, ip, "www-form", fp);
...
void service_http_form(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname, char *type, ptr_header_node *ptr_head, ptr_cookie_node *ptr_cookie)
{
  ...
  while (1)
  {
    ...
    case 1: /* connect and service init function */
    {
      ...
      if (sock < 0)
      {
        fpassword_report(stderr, "[ERROR] Child with pid %d terminating, cannot connect\n", (int32_t)getpid());
        fpassword_child_exit(1);
      }
      next_run = 2;
      break;
...
void service_http_get_form(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname)
{
  ...
  {
    fpassword_report(stderr, "[ERROR] Could not launch head. Error while initializing.\n");
...
ptr_header_node initialize(char *ip, unsigned char options, char *miscptr)
{
  ...
  if (cmdlinetarget != NULL)
    ...
    webtarget = fpassword_address2string(ip);
  if (port != 0)
    webport = port;
  ...
  if (use_proxy == 1 && proxy_authentication[selected_proxy] != NULL)
  {
    ...
    add_header(&ptr_head, "User-Agent", "Mozilla 5.0 (Fpassword Proxy Auth)", HEADER_TYPE_DEFAULT);
    ...
    {
      fpassword_report(stderr, "Out of memory for \"Proxy-Authorization\" header.\n");
      return NULL;
    }
    if (getcookie)
    {
  ...
  else
  {
    if (use_proxy == 1)
    {
      // proxy without authentication
      add_header(&ptr_head, "Host", webtarget, HEADER_TYPE_DEFAULT);
      add_header(&ptr_head, "User-Agent", "Mozilla/5.0 (Fpassword Proxy)", HEADER_TYPE_DEFAULT);
      if (getcookie)
      {
        // doing a GET to get cookies
        if (cookie_request != NULL)
    ...
    else
    {
      // direct web server, no proxy
      add_header(&ptr_head, "Host", webtarget, HEADER_TYPE_DEFAULT);
      add_header(&ptr_head, "User-Agent", "Mozilla/5.0 (Fpassword)", HEADER_TYPE_DEFAULT);
...
void usage_http_form(const char *service)
{
  printf("Module %s requires the page and the parameters for the web form.\n\n"
         ...
         "                 regardless it's already being sent by Fpassword or not.\n"
         ...
         "fpassword as option separators.\n"
         "\nExamples:\n"
