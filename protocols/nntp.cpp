...
int32_t start_nntp(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp) {
  ...
  while (i > 0 && fpassword_data_ready(s) > 0)
    i = fpassword_recv(s, buffer, 300);
  ...
  case AUTH_LOGIN:
    ...
    if (buf == NULL || strstr(buf, "383") == NULL) {
      fpassword_report(stderr, "[ERROR] NNTP LOGIN AUTH : %s\n", buf);
      free(buf);
      return 3;
    }
    free(buf);
  ...
  case AUTH_PLAIN:
    ...
    if ((buf = fpassword_receive_line(s)) == NULL)
      return 1;
    if (buf == NULL || strstr(buf, "383") == NULL) {
      fpassword_report(stderr, "[ERROR] NNTP PLAIN AUTH : %s\n", buf);
      free(buf);
      return 3;
    }
    free(buf);
  ...
  case AUTH_CRAMMD5: {
    ...
    if ((buf = fpassword_receive_line(s)) == NULL)
      return 1;
    if (buf == NULL || strstr(buf, "383") == NULL) {
      fpassword_report(stderr, "[ERROR] NNTP CRAM-MD5 AUTH : %s\n", buf);
      free(buf);
      return 3;
    }
  ...
  case AUTH_DIGESTMD5: {
    ...
    if (buf == NULL || strstr(buf, "383") == NULL || strlen(buf) < 8) {
      fpassword_report(stderr, "[ERROR] NNTP DIGEST-MD5 AUTH : %s\n", buf);
      free(buf);
      return 3;
    }
    memset(buffer, 0, sizeof(buffer));
  ...
  case AUTH_NTLM: {
    ...
    if ((buf = fpassword_receive_line(s)) == NULL)
      return 1;
    if (buf == NULL || strstr(buf, "383") == NULL || strlen(buf) < 8) {
      fpassword_report(stderr, "[ERROR] NNTP NTLM AUTH : %s\n", buf);
      free(buf);
      return 3;
    }
    // recover challenge
  ...
  default: {
    ...
    if (buf[0] != '3') {
      if (verbose || debug)
        fpassword_report(stderr, "[ERROR] Not an NNTP protocol or service shutdown: %s\n", buf);
      free(buf);
      return (3);
    }
    free(buf);
  ...
  if (buf[0] == '2') {
    fpassword_report_found_host(port, ip, "nntp", fp);
...
void service_nntp(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  while (1) {
    ...
    case 1: /* connect and service init function */
      ...
      if (fpassword_send(sock, buffer1, strlen(buffer1), 0) < 0)
        fpassword_child_exit(2);
      ...
      */
...
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "SASL\\s.*NTLM")) {
#else
...
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "SASL\\s.*DIGEST-MD5")) {
#else
...
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "SASL\\s.*CRAM-MD5")) {
#else
...
#endif
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "SASL\\s.*PLAIN")) {
#else
...
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "SASL\\s.*LOGIN")) {
#else
...
#ifdef HAVE_PCRE
      if (fpassword_string_match(buf, "AUTHINFO\\sUSER")) {
#else
      ...
      if (verbose) {
        switch (nntp_auth_mechanism) {
        case AUTH_CLEAR:
          fpassword_report(stderr, "[VERBOSE] using NNTP AUTHINFO USER mechanism\n");
          break;
        case AUTH_LOGIN:
          fpassword_report(stderr, "[VERBOSE] using NNTP LOGIN AUTH mechanism\n");
          break;
        case AUTH_PLAIN:
          fpassword_report(stderr, "[VERBOSE] using NNTP PLAIN AUTH mechanism\n");
          break;
#ifdef LIBOPENSSL
        case AUTH_CRAMMD5:
          fpassword_report(stderr, "[VERBOSE] using NNTP CRAM-MD5 AUTH mechanism\n");
          break;
        case AUTH_DIGESTMD5:
          fpassword_report(stderr, "[VERBOSE] using NNTP DIGEST-MD5 AUTH mechanism\n");
          break;
#endif
        case AUTH_NTLM:
          fpassword_report(stderr, "[VERBOSE] using NNTP NTLM AUTH mechanism\n");
          break;
        }
      }
      usleepn(25);
