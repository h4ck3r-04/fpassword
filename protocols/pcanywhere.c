...
int32_t send_cstring(int32_t s, char *crypted_string)
{
  ...
  return fpassword_send(s, buffer2, 2 + strlen(crypted_string), 0);
}
...
int32_t start_pcanywhere(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp)
{
  ...
  for (i = 0; i < 4; i++)
  {
    if (fpassword_send(s, client[i], clientsize[i], 0) < 0)
    {
      return 1;
    }
  ...
  else
  {
    ...
    fpassword_report_found_host(port, ip, "pcanywhere", fp);
