...
char *message2buffer(struct rmessage *msg) {
  ...
  case 0x09:
    ...
    *((int32_t *)(data + 5)) = htonl(msg->checksum);
    ...
    break;
  default:
    fpassword_report(stderr, "unknown rmessage type\n");
    fpassword_child_exit(0);
    return NULL;
  }
  return data;
...
void service_radmin2(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  while (1) {
    ...
    fpassword_send(sock, request, 10, 0);
    free(msg);
    free(request);
    ...
    fpassword_get_next_pair();
    strncpy(password, fpassword_get_next_password(), sizeof(password) - 1);
    ...
    request = message2buffer(msg);
    fpassword_send(sock, request, 42, 0);
    free(msg);
    free(request);
    ...
    case 0x0b:
      ...
      break;
    default:
      fpassword_report(stderr, "Error: Child with pid %d terminating, protocol error\n", (int32_t)getpid());
