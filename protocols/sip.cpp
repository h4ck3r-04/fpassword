    ...
    if (fpassword_data_ready_timed(s, 3, 0) > 0) {
      ...
      if (strncmp(buf, "SIP/2.0 606", 11) == 0) {
        ...
        if ((ptr = strstr(buf, "received="))) {
          ...
          if (verbose)
            fpassword_report(stderr, "[VERBOSE] Will reconnect using external IP address %s\n", external_ip_addr);
          return 1;
        }
        fpassword_report(stderr, "[ERROR] Could not find external IP address in server answer\n");
        return 2;
      }
    }
  }
