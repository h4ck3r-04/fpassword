...
int32_t start_s7_300(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp)
{
  ...
  if (fpassword_send(s, (char *)p_cotp, 22, 0) < 0)
    return 1;
  memset(buffer, 0, sizeof(buffer));
  ret = fpassword_recv_nb(s, buffer, sizeof(buffer));
  ...
  if (fpassword_send(s, (char *)p_s7_negotiate_pdu, 25, 0) < 0)
    return 1;
  memset(buffer, 0, sizeof(buffer));
  ret = fpassword_recv_nb(s, buffer, sizeof(buffer));
  ...
  if (fpassword_send(s, (char *)p_s7_read_szl, 33, 0) < 0)
    return 1;
  memset(buffer, 0, sizeof(buffer));
  ret = fpassword_recv_nb(s, buffer, sizeof(buffer));
  ...
  if (fpassword_send(s, buffer, 29 + S7PASSLEN, 0) < 0)
    return 1;
  ...
  if (ret > 30)
  {
    ...
    {
      fpassword_report_found_host(port, ip, "s7-300", fp);
    ...
    if (buffer[27] == '\xd6' && buffer[28] == '\x05')
    {
      // fpassword_report_found_host(port, ip, "s7-300", fp);
      fpassword_completed_pair_found();
      fpassword_report(stderr, "[INFO] No password protection enabled\n");
      if (memcmp(fpassword_get_next_pair(), &FPASSWORD_EXIT, sizeof(FPASSWORD_EXIT)) == 0)
        return 2;
      return 1;
    }
...
void service_s7_300(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname)
{
  ...
  while (1)
  {
    ...
    case 1: /* connect and service init function */
      sock = fpassword_connect_tcp(ip, s7port);
      if (sock < 0)
      {
...
int32_t service_s7_300_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname)
{
  ...
  // send p_cotp and check first 2 bytes of answer
  if (fpassword_send(sock, (char *)p_cotp, 22, 0) < 0)
  {
    fprintf(stderr, "[ERROR] can not send data to service\n");
    return 3;
  }
  memset(buffer, 0, sizeof(buffer));
  if ((ret = fpassword_recv_nb(sock, buffer, sizeof(buffer))) <= 0)
  {
  ...
  // send p_s7_negotiate_pdu and check first 2 bytes of answer
  if (fpassword_send(sock, (char *)p_s7_negotiate_pdu, 25, 0) < 0)
  {
  ...
  // send p_s7_read_szl and check first 2 bytes of answer
  if (fpassword_send(sock, (char *)p_s7_read_szl, 33, 0) < 0)
  {
  ...
  if (fpassword_send(sock, buffer, 29 + S7PASSLEN, 0) < 0)
  {
  ...
  if (ret > 30)
  {
    ...
    {
      fpassword_report(stderr, "[INFO] No password protection enabled, no password "
                               "tests are necessary!\n");
      return 1;
    }
  }
