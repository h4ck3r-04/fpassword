...
int32_t start_imap(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp) {
  ...
  case AUTH_CRAMSHA256: {
    ...
    if (strstr(buf, " NO ") != NULL || strstr(buf, "failed") != NULL || strstr(buf, " BAD ") != NULL || strstr(buf, "BYE") != NULL) {
      ...
      case AUTH_CRAMMD5:
        ...
        break;
      case AUTH_CRAMSHA1:
        fpassword_report(stderr, "[ERROR] IMAP CRAM-SHA1 AUTH : %s\n", buf);
        break;
      case AUTH_CRAMSHA256:
        fpassword_report(stderr, "[ERROR] IMAP CRAM-SHA256 AUTH : %s\n", buf);
        break;
      }
      free(buf);
      return 3;
  ...
  case AUTH_SCRAMSHA1: {
    ...
    snprintf(clientfirstmessagebare, sizeof(clientfirstmessagebare), "n=%s,r=fpassword", preplogin);
    free(preplogin);
    memset(buffer2, 0, sizeof(buffer2));
  ...
  fpassword_report_found_host(port, ip, "imap", fp);
...
void service_imap(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
  ...
  while (1) {
    ...
    case 1: /* connect and service init function */
      ...
      if (!disable_tls) {
        ...
        if (strstr(buf, "STARTTLS") != NULL) {
          ...
          if (buf == NULL || (strstr(buf, " NO ") != NULL || strstr(buf, "failed") != NULL || strstr(buf, " BAD ") != NULL)) {
            ...
            if (fpassword_send(sock, "3 CAPABILITY\r\n", strlen("3 CAPABILITY\r\n"), 0) < 0)
              fpassword_child_exit(2);
            buf = imap_read_server_capacity(sock);
            counter++;
            if (buf == NULL)
      ...
      if (verbose)
        fpassword_report(stderr, "[VERBOSE] CAPABILITY: %s", buf);
      ...
      if (verbose) {
        ...
        case AUTH_CLEAR:
          ...
          break;
        case AUTH_LOGIN:
          fpassword_report(stderr, "[VERBOSE] using IMAP LOGIN AUTH mechanism\n");
          break;
        case AUTH_PLAIN:
          fpassword_report(stderr, "[VERBOSE] using IMAP PLAIN AUTH mechanism\n");
          break;
#ifdef LIBOPENSSL
        case AUTH_CRAMMD5:
          fpassword_report(stderr, "[VERBOSE] using IMAP CRAM-MD5 AUTH mechanism\n");
          break;
        case AUTH_CRAMSHA1:
          fpassword_report(stderr, "[VERBOSE] using IMAP CRAM-SHA1 AUTH mechanism\n");
          break;
        case AUTH_CRAMSHA256:
          fpassword_report(stderr, "[VERBOSE] using IMAP CRAM-SHA256 AUTH mechanism\n");
          break;
        case AUTH_DIGESTMD5:
          fpassword_report(stderr, "[VERBOSE] using IMAP DIGEST-MD5 AUTH mechanism\n");
          break;
        case AUTH_SCRAMSHA1:
          fpassword_report(stderr, "[VERBOSE] using IMAP SCRAM-SHA1 AUTH mechanism\n");
          break;
#endif
        case AUTH_NTLM:
          fpassword_report(stderr, "[VERBOSE] using IMAP NTLM AUTH mechanism\n");
          break;
        }
      }
