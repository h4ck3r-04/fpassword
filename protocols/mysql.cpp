...
extern int32_t internal__fpassword_recv(int32_t socket, char *buf, int32_t length);
...
char *fpassword_mysql_receive_line(int32_t socket) {
  ...
  if (debug)
    fpassword_report_debug(stderr, "DEBUG_RECV_BEGIN|%s|END\n", buff);
  return buff;
}
...
char fpassword_mysql_init(int32_t sock) {
  ...
  buf = fpassword_mysql_receive_line(sock);
  if (buf == NULL)
    return 1;
...
char *fpassword_mysql_prepare_auth(char *login, char *pass) {
  ...
  *((unsigned long *)response) = response_len - 4;
  ...
  fpassword_scramble((char *)&response[9 + login_len + 1], mysqlsalt, pass);
...
int32_t start_mysql(int32_t sock, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp) {
  ...
  if (res == 2) {
    ...
    if (!mysql_real_connect(mysql, fpassword_address2string(ip), login, pass, database, port, NULL, 0)) {
      ...
      if (debug)
        fpassword_report(stderr, "[ERROR] Failed to connect to database: %s\n", mysql_error(mysql));
  ...
  response = fpassword_mysql_prepare_auth(login, pass);
  if (response == NULL)
    return 3;
  ...
  if ((response = fpassword_mysql_receive_line(sock)) == NULL)
    return 1;
...
char *fpassword_scramble(char *to, const char *message, const char *password) {
  ...
  if (password && password[0]) {
    ...
    fpassword_hash_password(hash_message, message);
    fpassword_randominit(&rand_st, hash_pass[0] ^ hash_message[0], hash_pass[1] ^ hash_message[1]);
    while (*message++)
      *to++ = (char)(floor(fpassword_rnd(&rand_st) * 31) + 64);
    extra = (char)(floor(fpassword_rnd(&rand_st) * 31));
    while (to_start != to)
      *(to_start++) ^= extra;
  }
  *to = 0;
